...
export function ContactForm() {
  ...
  if (submitted) {
    return (
      <motion.div initial={{ scale: 0.94, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center max-w-md mx-auto px-4">
        ...
        <Link href="/" className="btn-premium inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[var(--accent-green)] text-white font-medium hover:opacity-90 transition-opacity text-sm shadow-lg shadow-[var(--accent-green)]/20">
          Back to Lumivra
        </Link>
      </motion.div>
    )
  ...
  return (
    <motion.form initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} onSubmit={handleSubmit} className="premium-surface bg-[var(--card)] border border-[var(--border)] rounded-3xl p-6 sm:p-8 space-y-5 shadow-sm">
      <div>
      ...
      <div>
        ...
        <select id="subject" required value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} className="w-full bg-[var(--muted)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm text-[var(--foreground)] outline-none focus:ring-2 focus:ring-[var(--accent-green)] transition appearance-none cursor-pointer hover:border-[var(--accent-green)]/40">
      ...
      <div>
        ...
        <textarea id="message" required rows={6} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="Tell us what's on your mind..." className="w-full bg-[var(--muted)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] outline-none focus:ring-2 focus:ring-[var(--accent-green)] transition resize-none hover:border-[var(--accent-green)]/40" />
      </div>
      <div className="premium-surface bg-[var(--muted)] border border-[var(--border)] rounded-xl p-4">
        <label className="flex gap-3 cursor-pointer">
          ...
          <span className="text-xs text-[var(--muted-foreground)] leading-relaxed">I have read and agree to the <Link href="/privacy" className="text-[var(--accent-green)] underline underline-offset-2">Privacy Policy</Link>. I understand my personal data will be processed by KM TECH LABS solely to respond to this enquiry and deleted within 12 months. I can request erasure at any time. <span className="text-red-400">*</span></span>
        </label>
      </div>
      <button type="submit" disabled={loading || !form.consent} className="btn-premium shimmer-on-hover w-full flex items-center justify-center gap-2 py-3 px-6 rounded-xl bg-[var(--accent-green)] text-white font-medium hover:opacity-90 transition-opacity shadow-lg shadow-[var(--accent-green)]/20 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div className="grid sm:grid-cols-2 gap-3 text-xs text-[var(--muted-foreground)]">
        <div className="premium-surface hover-lift flex items-center gap-2 bg-[var(--muted)] border border-[var(--border)] rounded-xl px-3 py-2"><Clock className="w-3.5 h-3.5 text-[var(--accent-green)]" /> Response within 2 business days</div>
        <div className="premium-surface hover-lift flex items-center gap-2 bg-[var(--muted)] border border-[var(--border)] rounded-xl px-3 py-2"><Shield className="w-3.5 h-3.5 text-[var(--accent-green)]" /> GDPR requests answered within 30 days</div>
      </div>
    </motion.form>
  )
}
